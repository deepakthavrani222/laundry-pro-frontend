...
export default function CenterAdminServicesPage() {
  ...
  const getAuthToken = () => {
    // First check center admin store
    try {
    ...
    return localStorage.getItem('token') || localStorage.getItem('center-admin-token')
  }
  ...
  const fetchServices = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/center-admin/services', {
        headers: {
  ...
  const fetchBranches = async () => {
    try {
      const token = getAuthToken()
      const response = await fetch('http://localhost:5000/api/center-admin/branches', {
        headers: {
          'Content-Type': 'application/json',
          ...(token && { Authorization: `Bearer ${token}` })
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = getAuthToken()
      const url = isEditing 
        ? `http://localhost:5000/api/center-admin/services/${selectedService?._id}`
        : 'http://localhost:5000/api/center-admin/services'
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = getAuthToken()
      const response = await fetch(`http://localhost:5000/api/center-admin/services/${deleteServiceId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          ...(token && { Authorization: `Bearer ${token}` })
  ...
  const handleToggleStatus = async (service: Service) => {
    try {
      const token = getAuthToken()
      const response = await fetch(`http://localhost:5000/api/center-admin/services/${service._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          ...(token && { Authorization: `Bearer ${token}` })
  ...
  const handleAssignBranch = async (serviceId: string, branchId: string, isActive: boolean) => {
    ...
    try {
      const token = getAuthToken()
      const response = await fetch(`http://localhost:5000/api/center-admin/services/${serviceId}/branches`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(token && { Authorization: `Bearer ${token}` })
