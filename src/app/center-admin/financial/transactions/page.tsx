...
export default function TransactionsPage() {
  ...
  const handleApproveRefund = async (transactionId: string) => {
    try {
      ...
      await centerAdminApi.approveRefund(transactionId)
  ...
  const handleRejectRefund = async (transactionId: string, reason: string) => {
    try {
      ...
      await centerAdminApi.rejectRefund(transactionId, reason)
      await fetchTransactions()
    } catch (err: any) {
      setError(err.message)
    } finally {
                ...
                <tr key={transaction._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end gap-2">
                      <Link
                        href={`/center-admin/financial/transactions/${transaction._id}`}
