...
export default function CenterAdminLogin() {
  ...
  const { setAdmin, setToken, setSession } = useCenterAdminStore()
  ...
  const handleCredentialsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/center-admin/auth/login', {
        method: 'POST',
        headers: {
  ...
  const handleMFASubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/center-admin/auth/verify-mfa', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <h2 className="text-3xl font-bold text-white">Center Admin</h2>
        ...
        <div className="text-center">
          <p className="text-gray-400 text-sm">
            LaundryPro Center Admin Portal
          </p>
