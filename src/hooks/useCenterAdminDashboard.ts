...
export const useCenterAdminDashboard = () => {
  ...
  const fetchDashboardOverview = useCallback(async (timeframe: string = '30d') => {
    ...
    try {
      const response = await centerAdminApi.getDashboardOverview(timeframe)
  ...
  const fetchAnalytics = useCallback(async (params: {
    ...
    try {
      const response = await centerAdminApi.getDetailedAnalytics(params)
